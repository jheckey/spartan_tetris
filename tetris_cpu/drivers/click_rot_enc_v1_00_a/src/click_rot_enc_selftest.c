/*****************************************************************************
* Filename:          /home/jheckey/Dropbox/homework/ece253/spartan_tetris/tetris_cpu/drivers/click_rot_enc_v1_00_a/src/click_rot_enc_selftest.c
* Version:           1.00.a
* Description:       Contains a diagnostic self-test function for the click_rot_enc driver
...
XStatus CLICK_ROT_ENC_SelfTest(void * baseaddr_p)
{
  ...
  CLICK_ROT_ENC_mWriteSlaveReg0(baseaddr, 0, 1);
  Reg32Value = CLICK_ROT_ENC_mReadSlaveReg0(baseaddr, 0);
  ...
  Reg32Value = CLICK_ROT_ENC_mReadReg(baseaddr, CLICK_ROT_ENC_INTR_IPISR_OFFSET);
  xil_printf("   - IP (user logic) interrupt status : 0x%08x\n\r", Reg32Value);
  xil_printf("   - clear IP (user logic) interrupt status register\n\r");
  CLICK_ROT_ENC_mWriteReg(baseaddr, CLICK_ROT_ENC_INTR_IPISR_OFFSET, Reg32Value);
  Reg32Value = CLICK_ROT_ENC_mReadReg(baseaddr, CLICK_ROT_ENC_INTR_DISR_OFFSET);
  xil_printf("   - Device (peripheral) interrupt status : 0x%08x\n\r", Reg32Value);
  xil_printf("   - clear Device (peripheral) interrupt status register\n\r");
  CLICK_ROT_ENC_mWriteReg(baseaddr, CLICK_ROT_ENC_INTR_DISR_OFFSET, Reg32Value);
  xil_printf("   - enable all possible interrupt(s)\n\r");
  CLICK_ROT_ENC_EnableInterrupt(baseaddr_p);
